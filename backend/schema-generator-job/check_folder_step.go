...
// CheckFolderStep checks if the required folder exists in the cloned repository.
type CheckFolderStep struct {
	destPath  string
	schemaIDs []string
}

// Do performs the step of checking the folder's existence.
func (s *CheckFolderStep) Do(ctx context.Context) error {
