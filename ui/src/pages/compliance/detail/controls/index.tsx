...
export default function Controls({ id, enable, title }: IPolicies) {
  ...
  return (
    <Grid numItems={12} className="gap-4 mt-1 sm:mt-4 ">
      {selectedBread && selectedBread?.length > 0 && (
        <Col numColSpan={12}>
          <BreadcrumbGroup
            onClick={(event) => {
              ...
              <>
                <Select
                  ...
                  onChange={({ detail }) => {
                    ...
                    setSelected(detail.selectedOption.href);
                    const temp = [];

                    if (detail.selectedOption.parentId) {
                      temp.push({
                        text: title,
                        href: id,
                      });
                      temp.push({
                        text: detail.selectedOption.parentTitle,
                        href: detail.selectedOption.parentId,
                      });
                      temp.push({
                        text: detail.selectedOption.label,
                        href: detail.selectedOption.value,
                      });
                    } else {
                      temp.push({
                        text: title,
                  ...
                  options={tree.map((tre, index) => {
                    return {
                      label: tre.text,
                      value: tre.href,
                      parentId: tre?.parentId,
                      parentTitle: tre?.parentTitle,
                    };
                  })}
